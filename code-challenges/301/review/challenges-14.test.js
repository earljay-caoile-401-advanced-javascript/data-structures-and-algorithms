...
let biggerThanLuke = (arr) => {
  ...
  const reducer = (acc, currObj) => {
    if (currObj.name === 'Luke Skywalker') {
      lukeVals = currObj;
    } else {
      if (parseInt(currObj.height) > parseInt(lukeVals.height)) {
        acc = acc ? `${acc} - ${currObj.name}` : currObj.name;
      }
    }
    return acc;
  };
...
const detectTicTacToeWin = (board) => {
  ...
  const diagonalWin = (board) => {
    const topLeftToBotRight = !!board[0][0] && board[0][0] == board[1][1] && board[1][1] === board[2][2];
    const botLeftToTopRight = !!board[0][2] && board[0][2] == board[1][1] && board[1][1] === board[2][0];
