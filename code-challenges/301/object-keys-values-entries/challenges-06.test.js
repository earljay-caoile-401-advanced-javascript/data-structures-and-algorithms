...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(oneChar => {
    if (!hasChildren) {
      Object.values(oneChar).forEach(value => {
        if (value === character) {
          hasChildren = oneChar.children.length > 0;
        }
      });
    }
  });
...
const hasChildrenEntries = (arr, character) => {
  let hasChildren = false;
  arr.forEach(oneChar => {
    if (!hasChildren && oneChar.name === character) {
      hasChildren = oneChar.children.length > 0;
    }
  });
  return hasChildren;
...
const totalCharacters = arr => {
  ...
  arr.forEach(oneChar => {
    ...
    if (oneChar.children.length) {
      oneChar.children.forEach(() => {
        total++;
      });
    }
  });
...
const houseSurvivors = arr => {
  ...
  arr.forEach(oneChar => {
    let total = 0;
    total++;
    if (oneChar.spouse && !deceasedSpouses.includes(oneChar.spouse)) {
      total++;
    }
    if (oneChar.children.length) {
      oneChar.children.forEach(() => {
