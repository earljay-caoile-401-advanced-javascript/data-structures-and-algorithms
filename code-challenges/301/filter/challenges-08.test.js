...
const notInFirstArray = (forbiddenValues, arr) => {
  const forbiddenSet = new Set(forbiddenValues);
  return arr.filter(num => !forbiddenSet.has(num));
};
...
const getStatName = (arr, minBaseStat) => {
  return arr.filter(item => item.baseStat > minBaseStat).map(obj => obj.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  return arr.filter(num => typeof num === 'number').map(num => num % 2 === 0 ? 'even' : 'odd');
};
