...
const count = (target, input) => {
  ...
  return input.filter(arr => arr.includes(target)).reduce(reducer, 0);
};
...
const totalSum = (input) => {
  const calcSum = (currSum, currArr) => {
    currArr.forEach(num => currSum += num);
    return currSum;    
  };
  return input.reduce(calcSum, 0);
};
...
let findMaleAndFemale = (data) => {
  const reducer = (acc, currObj) => {
    return acc ? `${acc} and ${currObj.name}` : currObj.name; 
  };
  return data.filter(char => char.gender === 'male' || char.gender === 'female').reduce(reducer, '');
};
...
let findShortest = (data) => {
  const reducer = (currShortest, currObj) => parseInt(currShortest.height) < parseInt(currObj.height) ? currShortest : currObj;
  return data.reduce(reducer).name;
};
