...
//   const reducer = (acc, currArr) => {
//     const arrToAdd = new Array();
//     currArr.forEach(name => {
...
//     return acc;
//   };
//   return arr.reduce(reducer, new Array());
// };
const unenrollBrook = (arr) => {
...
const sortByDay = (arr) => {
  ...
  const reducer = (acc, currStr) => {
    let i = 0;
    daysOfWeek.forEach(day => {
      const tmpRegex = new RegExp(`.*${day}.*`);
      if (tmpRegex.test(currStr)) {
        acc[i].push(currStr);
      }
      i++;
    });
