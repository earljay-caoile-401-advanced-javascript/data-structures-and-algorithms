...
const returnNames = (arr) => {
  const res = new Array();
  const reducer = (accumulator, currentValue) => res.push(currentValue.name);
  arr.reduce(reducer, res);
  return res;
};
...
const reversedString = (str) => {
  const res = new Array();
  const reducer = (accumulator, currentValue) => res.splice(0, 0, currentValue);
  str.split('').reduce(reducer, res);
  return res.join('');
};
...
const countNumberOfChildren = (arr) => {
  const reducer = (accumulator, currentValue) => accumulator = currentValue.children ? accumulator + currentValue.children.length : accumulator;
  return arr.reduce(reducer, 0);
};
...
const calculateAverage = (arr) => {
  const defaultVals = { count: 0, sum: 0 };
  const reducer = (accumulator, currentValue) => {
    accumulator.sum += currentValue;
    accumulator.count++;
    return accumulator;
  };
  arr.reduce(reducer, defaultVals);
  return defaultVals.count === 0 ? 0 : defaultVals.sum / defaultVals.count;
};
...
const countPrimeNumbers = (arr) => {
  const reducer = (accumulator, currentValue) => isPrime(currentValue) ? accumulator + 1 : accumulator;
  return arr.reduce(reducer, 0);
};
...
const extractStat = (statName, arr) => {
  const reducer = (accumulator, currentValue) => currentValue.name === statName ? currentValue : accumulator;
...
const extractChildren = (arr) => {
  // const res = new Array();
  const filtArr = arr.filter(char => char.name.includes('a'));
  const reducer = (accumulator, currentValue, currentIndex, array) =>{
    if (array[currentIndex].children) {
      array[currentIndex].children.forEach(child => {
        accumulator.push(child);
      });
    }
    return accumulator;
  };
  return filtArr.reduce(reducer, new Array());
};
